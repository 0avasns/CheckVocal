...
DEFAULT_ENTERSTRING = False  # user-supplied string instead of correct/wrong # ThP April 2019
DEFAULT_DOCLASSIFY = False  # classification into N groups instead of correct/wrong # ThP May 2019
DEFAULT_NCLASSES = 3  # classification into N groups instead of correct/wrong # ThP May 2019
DEFAULT_MODECHOICE = 1  # default correct/wrong classification
DEFAULT_SEP = 0  # separator for output data file defaults to Tab (1=space, 2=comma)
DEFAULT_SAVEDATE = 1 # optionally save date/time/PC info along with output data
DEFAULT_SAVETIME = 1
...
##
DMDXMODE = True  # True when processing azk files; False when processing arbitrary audio file sets
...
class GlobVariables:
    ...
    def __init__(self):
        ...
        self.enterstring = IntVar(root)   # ThP April 2019
        self.userstring = StringVar(root) # ThP April 2019
        self.doclassify = IntVar(root)    # ThP May 2019
        self.modechoice = IntVar(root)    # ThP May 2019
        self.nclasses = StringVar(root)   # ThP May 2019
        ...
        self._CLASSMULT = DEFAULT_CLASSMULT
        #
        ## Fonts
    ...
    def reset(self):
        ...
        self.enterstring.set(DEFAULT_ENTERSTRING)  # ThP April 2019
        self.userstring.set("")  # ThP April 2019
        self.doclassify.set(DEFAULT_DOCLASSIFY)  # ThP May 2019
        self.modechoice.set(DEFAULT_MODECHOICE)  # ThP May 2019
...
class SetupWindow(Toplevel):
    ...
    def pack_workmode_row(self):
        self.modF2a = Frame(self)
        self.rlabel2a = Label(self.modF2a, text="Work mode:")
        self.rlabel2a.config(width=20, font=gv.mainfont, anchor="e")
        self.rlabel2a.pack(side="left")
        self.modechoice2a = Radiobutton(self.modF2a, text="Correct/Wrong", variable=gv.modechoice, value=1, command=self.classifyoff)
        self.modechoice2a.config(font=gv.mainfont)
        self.modechoice2a.pack(side="left", anchor="s")
        self.modechoice2b = Radiobutton(self.modF2a, text="Sort into", variable=gv.modechoice, value=2, command=self.classifyon)
        self.modechoice2b.config(width=7, font=gv.mainfont)
        self.modechoice2b.pack(side="left", anchor="s")
        self.spinn2a = Spinbox(self.modF2a, values=(2, 3, 4, 5, 6))
        self.spinn2a.config(width=1, font=gv.mainfont, textvariable=gv.nclasses, state="disabled")
        self.spinn2a.pack(side="left")
        self.rlabel2b = Label(self.modF2a, text="classes")
        self.rlabel2b.config(width=8, font=gv.mainfont, anchor="w")
        self.rlabel2b.pack(side="left")
        self.modechoice2c = Radiobutton(self.modF2a, text="Enter number", variable=gv.modechoice, value=3, command=self.classifyoff)
        self.modechoice2c.config(width=10, font=gv.mainfont)
        self.modechoice2c.pack(side="left", anchor="s")
        self.modF2a.pack(side="bottom", anchor="w")

    def classifyon(self):
        gv.enterstring.set(False)
        self.activate_voxchoice()
        gv.doclassify.set(True)
        self.spinn2a.config(state="normal")

    def classifyoff(self):
        if (gv.modechoice.get() == 3):
            gv.enterstring.set(True)
            self.deactivate_voxchoice()
        else:
            gv.enterstring.set(False)
            self.activate_voxchoice()
        gv.doclassify.set(False)
        self.spinn2a.config(state="disabled")

    def deactivate_voxchoice(self):
        self.trigchoice1.config(state="disabled")
        self.trigchoice2.config(state="disabled")
        self.filtdcC.config(state="disabled")
        self.revtrigC.config(state="disabled")
        self.rmslim31.config(state="disabled")
        self.rmslim32.config(state="disabled")

    def activate_voxchoice(self):
        self.trigchoice1.config(state="normal")
        self.trigchoice2.config(state="normal")
        self.filtdcC.config(state="normal")
        self.revtrigC.config(state="normal")
        self.rmslim31.config(state="normal")
        self.rmslim32.config(state="normal")
        if (gv.trigchoice.get()==0): self.trigdmdx()
        elif (gv.trigchoice.get()==1): self.trignew()
    ...
    def inrun(self):
        ...
        self.modechoice2c.config(state=DISABLED)
        self.spinn2a.config(state=DISABLED)
        #
...
class CheckWaves(Toplevel):
    ...
    def goback(self, event=None):  # go to the previous file
        if ((not gv._ENTERSTRING) or (gv._ENTERSTRING and self.TextEnter())): # don't go back with an invalid text field
            ...
            if (cv_process.current_index >= 0):  # not reached the beginning of the list
                ...
                cv_process.N_done -= 2
                self.advance_file(fromgoback=True)
            else:
                cv_process.current_index = save_index  # do not go back if there are no previous trials
                self.gobackb.config(state=DISABLED)

    def get_itemrt(self,new=False):
        if new:
            self.item, selfrt = gv.listoftrials[cv_process.current_index]
        else:
            self.item, selfrt = gv.sub_trials[gv.subj_ind[cv_process.current_index]][gv.trial_ind[cv_process.current_index]]
        if gv._DOCLASSIFY:
            self.rt = selfrt % gv._CLASSMULT
            self.nclass = selfrt // gv._CLASSMULT
        else:
            self.rt = selfrt
            self.nclass = 0
        self.get_absrt()

    def get_absrt(self): # define a separate function so that it can also be called upon updates
        if self.rt < 0:
            self.signrt = -1
            self.absrt = -self.rt
        else:
    ...
    def update_itemrt(self,newrt):
        if ( abs(newrt) >= gv._CLASSMULT ): exiterror("Internal error: Augmented RT (%d) in update_itemrt"%(newrt))
        self.rt = newrt
        self.get_absrt()
        if gv._DOCLASSIFY:
    ...
    def timedout(self, event=None):  # mark this file as not containing a response (timed out)
        ...
        if (gv._ENTERSTRING):
            signrt = -1  # always negative, to make sure it stands out for user-entered numbers
            gv.userstring.set("")
        elif (self.newlabel == "---"):  # correct to not respond
            signrt = 1
        else:  # failed to utter a  response when one was appropriate
            signrt = -1
        # if gv._DOCLASSIFY:
        #     newrt = gv.timeout + self.nclass * gv._CLASSMULT
        # else:
        #     newrt = gv.timeout
        newrt = gv.timeout * signrt
        self.update_itemrt(newrt)
        logmsg("%s contains no response, changing RT to %.1f" % (gv.listoffiles[cv_process.current_index], newrt))
        self.advance_file()

    def advance_file(self, event=None, fromgoback=False):  # prepare to load next file and update status
        if (fromgoback or (not gv._ENTERSTRING) or (gv._ENTERSTRING and self.TextEnter())):
            self.hide_lines()
    ...
    def load_new_file(self):  # load new sound file and update display
        ...
        self.zoomall()  # use zoom-all instead of drawing wave and lines separately; this also takes care of updating the display fully
        if gv._REVERSETRIGGER:
            self.playleft()
        else:
            self.playright()
    ...
    def revertRT(self, event=None):
        ...
        #self.get_itemrt() # this should not be necessary
        # if (self.rt != 0):
        #     signrt = self.rt / abs(self.rt)  # retain the sign regardless of RT value
        # else:
        #     signrt = 1.0
        newrt = self.signrt * abs(self.remember_rt)
        self.draw_lines()
        if gv._REVERSETRIGGER:
            self.playleft()
        else:
    ...
    def mouseclick(self, event):  # update RT with position of mouse click
        ...
        if (0 < x < gv._C_WIDTH and 0 < y < (2 * gv._C_HEIGHT + gv._C_DIST)):
            #self.get_itemrt() # this shold not be necessary
            # if (self.rt != 0):
            #     signrt = self.rt / abs(self.rt)
            # else:
            #     signrt = 1.0
    ...
    def TextEnter(self, event=None):
        ...
        if (ustr.isdigit() and (len(ustr) < 10)):  # store number
            gv.listofproblems.append(gv.listoffiles[cv_process.current_index])
            newrt = int(ustr)
            if (newrt != self.rt):  # only update if different
                self.update_itemrt(newrt)
    ...
    def classifyN(self, N, event=None):  # code based on snd_ok ; multiply RT by N million
        ...
        gv.listofproblems.append(gv.listoffiles[cv_process.current_index])
        self.nclass = N
        self.update_itemrt(self.rt)
        logmsg("%s is classifed as %d, changing RT to %.1f" % (gv.listoffiles[cv_process.current_index], self.nclass, self.rt+N*gv._CLASSMULT))
        self.advance_file()
    ...
    def __init__(self, parent):
        ...
        self.newlabel = ""
        #### keyboard shortcuts
        if (not (gv._ENTERSTRING or gv._DOCLASSIFY)):
            self.bind("c", self.snd_ok)
            self.bind("v", self.not_ok)
        if gv._DOCLASSIFY:
            for ic in range(gv._NCLASSES):
                ## https://stackoverflow.com/questions/6920302/how-to-pass-arguments-to-a-button-command-in-tkinter
                self.bind("%1d"%(ic+1), functools.partial(self.classifyN,ic+1))
        ...
        if (gv._ENTERSTRING):
            Label(self.buttonrow1, text="User text:", font=gv.mainfont).pack(side=LEFT)
            self.utxt = Entry(self.buttonrow1, textvariable=gv.userstring, width=10, font=gv.mainfont)
            self.utxt.bind('<Return>', self.advance_file)  # <Return> or <Key-Return>
            self.utxt.pack(side='left', padx=1)
        elif (gv._DOCLASSIFY):
            for ic in range(gv._NCLASSES):
                ## https://stackoverflow.com/questions/6920302/how-to-pass-arguments-to-a-button-command-in-tkinter
                Button(self.buttonrow1, text=" [ %1d ] "%(ic+1), font=gv.largeboldfont, fg='#1111AA',command=functools.partial(self.classifyN,ic+1) ).pack(side='left', padx=1)
        else:
...
class CheckVocalClass:
    ...
    def process_azk(self):
        ...
        for subj in range(gv.Nsubj):
            ...
            if (gv._ENTERSTRING):
                tmptimes = [(x[0], -1) for x in
                            tmptrials]  # initially set all trials to -1 if set to custom string (ignore RTs)
            else:
            ...
            if (self.ntrials > 0):  # skip check for first subject
                if (endline - startline - exlines != self.ntrials):
                    logmsg("Mismatching number of trials (expected %i, found %i) for subject %i (ID %s)" % (
                    self.ntrials, endline - startline - exlines, subjno, s_id))
                    self.subject_select[s_id] = 0  # definitely kill the subject with unparseable data
            else:
        ...
        if (saved_session == 1):
            ...
            if (ans == -1):  # QUIT
                ...
                try:  # you never know when someone will try to process a folder on a CD...
                    ...
                    logmsg("Unable to open status files %s and %s for writing" % (
                    self.statusfilename1, self.statusfilename2))
            ...
            if gv.savetrialorder.get() == 1:
                for trial in gv.sub_trials[ref_subj]:
                    ...
                    for subjnum in live_subjects:
                        cur_subj = gv.sub_ids[subjnum]
                        outfile.write(
                            gv._SEP + "%i" % (gv.sub_order[cur_subj][gv.sub_trials[ref_subj].index(trial)][1]))
                    outfile.write("\n")
...
class CheckVocalClass_Files(CheckVocalClass):
    ...
    def process_azk(self):
        ...
        self.subject_select[s_id] = 1
        for trial in range(self.ntrials):
            gv.listoftrials.append((trial + 1, -1))  # dummy item number, dummy RT // no sorting necessary ## ThP 29May19 changed to -1 to work with text field
...
def global_quit():
    ...
    root.quit()
    time.sleep(0.5)
    raise SystemExit # need explicit exit because cv_process is not a Tk widget and is not destroyed by killing root
